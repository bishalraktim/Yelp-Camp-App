	...
	async.waterfall([
		...
		function(token, user, done) {
			var smtpTransport = nodemailer.createTransport({
				service: 'Gmail',
				auth: {
					user: 'yelpcamp8@gmail.com',
					pass: process.env.GMAILPW
				}
			});
			var mailOptions = {
				to: user.email,
				from: 'yelpcamp8@gmail.com',
...
router.post('/reset/:token', function(req, res) {
	async.waterfall([
		...
		function(user, done) {
			...
			smtpTransport.sendMail(mailOptions, function(err) {
				req.flash('success', 'Success! Your password has been changed. Please check your email for the confirmation message.');
				done(err);
			});
		}
	], function(err) {
